...
var (
	...
	ErrSelfChild = errors.New("cannot move into its own subtree")
	// ErrValidation 入参非法；用 %w 包裹 *ValidationError 获取字段明细。
	ErrValidation = errors.New("docservice: validation failed")
)
...
func (e *VersionConflictError) Error() string {
	return fmt.Sprintf("docservice: version conflict, current HEAD=%s", e.HeadCommitID)
}
