...
- [x] T11.3 users tab 补 display_name/q 搜索/危险操作确认；dashboard 渲染 recent_docs + contributors
...
- [x] T15.1 useMediaQuery composable（matchMedia 不可用回退桌面）+ 顶栏移动端收纳为下拉菜单 + 文档树抽屉（SideTree 复用）+ 顶栏/主区密度微调
   验收: 窄屏桌面 nav 不渲染，汉堡开树抽屉、⋯菜单含全部入口；桌面 DOM 与现状一致（app-header.spec 零改动通过）
- [x] T15.2 DocView 目录抽屉（≥lg 侧栏，窄屏「目录」按钮开抽屉，跳转后收起）
   验收: 窄屏侧栏隐藏、toggle 开抽屉、抽屉内点击触发锚点滚动
...
## M20 核心路径 UX 与本地 OIDC 验收（2026-09-25）

- [x] T20.1 阅读、查找与新建：正文 H1 与元数据标题去重；面包屑仅显示父级；搜索等待文档树生成正确 slug 链接并提供明确提交按钮；新建表单补字段说明、必填与冲突反馈
- [x] T20.2 编辑与管理：编辑者可进入文档树管理；wikilink 补全插入正典 slug 路径；图片粘贴仅作用于正文；草稿状态本地化，保存失败保留编辑页；设置按用途分组并展示九项帮助与字段错误
- [x] T20.3 回收站：平铺列表标注子树归属和子项数；彻底删除须确认影响范围，取消不发请求，失败有提示；对应方案见 doc/06
- [x] T20.4 自动验收：公开页面 Playwright 冒烟与真实 OIDC 浏览器闭环分开运行；测试 IdP + 每次全新 SQLite，覆盖 admin/viewer JIT、editor 权限升级、建文、编辑、阅读、搜索、后台和回收站；策略见 doc/09
  验证：前端 177 项单测、构建、公开浏览器 2 项、OIDC 浏览器 1 项、`go test ./...`、`go vet ./...` 均通过。
  项目级遗留门禁：后端总覆盖率 76.8%，仍低于路线图 85% 临时门槛；生产真实 IdP/反代验收 T13.2 尚未完成。本批未改冻结契约。

## M21 打印、快捷键与站点时区（2026-09-25）

- [x] T21.1 站点时区：`GET /v1/site` 暴露全站 IANA 时区；未修改的数据库种子值跟随配置默认值，管理员在线修改优先；评论与版本历史按站点时区和界面语言显示，保存后当前页面即时更新
- [x] T21.2 文档打印：文档页打印入口与打印媒体样式；保留标题和正文，隐藏导航、侧栏和操作控件；暗色主题打印为白底黑字
- [x] T21.3 快捷键：Ctrl/⌘+Shift+F 进入搜索并聚焦输入；显示快捷键提示；打开的对话框和编辑页离开确认不被绕过；Esc 沿用 Element Plus 弹层默认行为和编辑器补全已有关闭行为
  验证：前端 180 项单测、构建、公开浏览器 4 项、OIDC 浏览器 1 项、`go test ./...` 与 `go vet ./...` 通过。项目级覆盖率和 T13.2 仍按 M20 遗留跟踪。
- [x] T21.4 打印分页回归修复（用户验收反馈）：长代码块允许跨页，打印时应用外层改为普通文档流；浏览器验收须实际生成多页 PDF 并检查首页含正文，不能仅断言打印媒体模式下元素可见
- [x] T21.5 打印独立页面（用户再次反馈 Edge 预览空白）：文档页打印按钮打开 `/print/docs/<slug路径>`；独立页面只加载正文，不挂载应用侧栏/弹层，等待增强渲染、字体和图片后再自动打印，保留手动重试按钮。用本机真实 `hello-world` 内容生成 PDF 验证首页含正文；公开浏览器测试覆盖按钮→新页→PDF 首页。

## M22 首页与常用操作 UX（2026-09-25）

- [x] T22.1 顶栏与文档操作层级：站点标题使用真实链接；桌面搜索保持可见；手机搜索和有权限的新建入口保持在顶栏；账号菜单收纳语言、主题、令牌、回收站与退出；匿名用户仍可调整显示偏好；文档页突出编辑和桌面历史，将导出、打印及窄屏历史收入「更多」；首页创建入口保留次级视觉权重；修正菜单嵌套交互控件和主题无障碍名称
  验证：前端 180 项单测、构建、公开浏览器 5 项、OIDC 浏览器 1 项通过；桌面与 375px 手机宽度检查权限入口及横向溢出。

## M23 编辑页窄屏源码优先（2026-09-25）

- [x] T23.1 编辑页在 1280px 以上默认源码与预览分栏；较窄视口默认显示完整宽度源码，「预览」切换到全宽预览；视口跨断点时同步切换模式；主内容允许收缩以避免横向裁切；空文档预览显示占位提示
  验证：前端 180 项单测、构建、OIDC 浏览器流程通过；浏览器检查 1280px 双栏、375px 源码宽度与预览切换，并确认主区域没有横向溢出。

## M24 站点时区预设（2026-09-26）

- [x] T24.1 管理设置的时区字段改为地区分组下拉预设，覆盖 UTC 与亚洲、欧洲、美洲、大洋洲常见时区；中英文标签显示当前 UTC 偏移和 IANA 标识，夏令时偏移按当前日期计算；站点现有的列表外时区保留为当前选项，可切换到预设
  验证：前端 181 项单测、构建与 OIDC 浏览器流程通过；端到端设置流程从下拉选择并保存 Europe/Berlin。

## M25 管理用户与仪表盘视觉整理（2026-09-26）

- [x] T25.1 Users 面板增加标题、说明、搜索状态、响应式可滚动表格、角色选择、启停状态徽标和空状态；Dashboard 面板改为响应式统计卡片、近期文档与贡献者面板，显示站点时区的更新时间；补齐中英文文案并适配深色模式
  验证：前端生产构建通过；本项仅调整管理页面呈现，未运行行为测试。
...
- C11 (2026-09-25): doc/02 §12 为 `GET /v1/site` 响应新增 `timezone`；明确在线修改优先于配置默认值，时间戳仍为 Unix 毫秒。获用户同意后先改契约，再实施 M21。
